...
线程拆分: 在 Minecraft.run() 开头，释放主线程的 GL 上下文，创建新的渲染线程来运行游戏循环

GL 上下文转移: Display.releaseContext() → Display.makeCurrent()

输入同步: 轮询线程排空 LWJGL 的 Mouse/Keyboard 事件队列到线程安全缓冲区，渲染线程从缓冲区读取

ASM 字节码修改: 通过 Forge 核心模组 (coremod) 修改 Mouse、Keyboard、Display 类的方法

安装方法

确保你已安装 Minecraft Forge 1.7.10（推荐 10.13.4.1614 或更高版本）

将 ixeris-1.7.10.jar 放入 .minecraft/mods/ 目录

启动 Minecraft

日志中应该能看到 [Ixeris] 开头的信息
